...
- **`games.krs`** — two sets: `games-sites` (domains, v2fly `category-games-!cn` — the overseas half only, see below) and `games-ip` (CIDRs, from 4 game-vendor ASNs — AS32590 Valve, AS6507 Riot, AS57976 Blizzard, AS33353 Sony/Gaikai). Split in two because game clients address login/patching/store by domain but reach match servers by bare IP with no DNS lookup — `games-sites` and `games-ip` need to be routable independently. Fail-closed: `validateGames` (`main.go`) aborts the build if `games-ip` comes back empty or stops covering the anchors in `gamesAnchors` (`sources.go`).

  **Client routing semantics**: referenced by `match.names` like the other standalone bundles — there is no implicit `via`, the client decides. `games-sites` is deliberately restricted to `category-games-!cn`: the CN half of v2fly's `category-games` (Tencent games, miHoYo's CN client, 4399, bilibili-game, etc. — ~289 domains) is already covered by `cn-sites` for direct routing, so pulling it into `games-sites` too would only create a route-order trap — whichever bundle a client evaluates first for those domains wins, and evaluating games first would silently strip domestic games off the direct route and send them overseas. Keeping `games-sites` overseas-only makes it purely additive: it can only ever add domains `cn-sites` doesn't already route, so route order can never take a CN game domain away from direct. Domestic games always stay on `cn-direct`.

  **Blast radius**: `validateGames` failing calls `log.Fatalf`, which aborts the *entire* daily build — not just games.krs. Every region (cn/ir/ru/...) skips that day's release; the CDN keeps serving the last good build. This is the same fail-closed tradeoff as `tencent-overseas` (deliberate — see "explicitly not doing" below): a single flaky game-vendor ASN feed freezes the whole fleet's rule updates for a day rather than risk silently shipping a games bundle that matches nothing.

  **`gamesAnchors` caveat (documented in full in `sources.go`)**: unlike `tencentOverseasAnchors` (real incident IPs), these are placeholders — the first advertised block from each ASN's current `ipverse/asn-ip` aggregated list, not an observed game server. The guard therefore only proves "the upstream ASN feed for this vendor is still reachable and still shaped like their block" — it does **not** prove that any specific address is still a live game server, or that the vendor still operates that block today beyond what the aggregated list currently claims. Upgrading each anchor to a packet-capture-observed IP (cross-checked at bgp.tools) would close that gap; not done yet. `gamesAnchors` must stay 1:1 with `games-ip`'s `IPURLs` (enforced by `games_test.go`) — otherwise the guard degrades from "every vendor ASN feed is alive" to "at least one is."
...
- **ipverse/asn-ip** — per-ASN aggregated CIDR lists, CC0 1.0; used for `tencent-overseas` and `games-ip`
